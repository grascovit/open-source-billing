...
module Reporting
  module Dashboard
    ...
    def self.get_recent_activity(currency=nil, company = nil)
      ...
      invoices.each { |inv| recent_activity << {:activity_type => "invoice", :activity_action => I18n.t('views.dashboard.sent_to'), :client => (inv.unscoped_client.organization_name rescue ''), :amount => inv.invoice_total, :unit => (inv.currency.present? ? inv.currency.unit : "USD"), :code => (inv.currency.present? ? inv.currency.code : "$"), :activity_date => inv.created_at, :activity_path => "/invoices/#{inv.id}/edit"} }
      payments.each { |pay| recent_activity << {:activity_type => "payment", :activity_action => I18n.t('views.dashboard.received_from'), :client => (pay.invoice.unscoped_client.organization_name rescue ''), :amount => pay.payment_amount, :unit => (pay.invoice.currency.present? ? pay.invoice.currency.unit : "USD"), :code => (pay.invoice.currency.present? ? pay.invoice.currency.code : "$"), :activity_date => pay.created_at, :activity_path => "/payments/#{pay.id}/edit"} }
