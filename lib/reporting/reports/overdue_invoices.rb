...
module Reporting
  module Reports
    class OverdueInvoices < Reporting::Report
      ...
      def get_report_data
        ...
        invoices = invoices.where(client: @report_criteria.client_id) unless @report_criteria.client_id == 0
        invoices = invoices.where('due_date < ?', Date.today)
        invoices.where.not(status: 'paid')
      end
      ...
      def invoices_collected_xlsx report
        doc = XlsxWriter.new
        doc.quiet_booleans!
        sheet1 = doc.add_sheet("Overdue Invoices")
